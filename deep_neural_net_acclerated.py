...
class NeuralNetwork:
    ...
    def __init__(self, 
        ...
        """
        self.wih = cp.random.normal(0.0, pow(self.inodes, -0.5), (self.hnodes[0], self.inodes))
        self.who = cp.random.normal(0.0, pow(self.hnodes[-1], -0.5), (self.onodes, self.hnodes[-1]))
        self.whh = {
            f"wh{i}h{i+1}" : cp.random.normal(0.0, pow(self.hnodes[i-1], -0.5), (self.hnodes[i], self.hnodes[i-1]))  
    ...
    def train(self, input_list, target_list):
        ...
        targets = cp.array(target_list, ndmin=2).T
        ...
        hidden_layer_1_inputs = cp.dot(self.wih, inputs)
        # print("hidden_layer_1_inputs: ", hidden_layer_1_inputs)
        # hidden_layer_1_inputs = hidden_layer_1_inputs.get()
        ...
        final_inputs = cp.dot(self.who, self.hidden_outputs[f'h{len(self.hnodes)}'])
        # final_inputs = final_inputs.get()
        ...
        hidden_error = cp.dot(self.who.T, output_error)
        self.hidden_errors[f'h{len(self.hnodes)}'] = hidden_error
        ...
        backprop_1 = cp.dot((output_error*final_outputs*(1.0-final_outputs)), cp.transpose(self.hidden_outputs[f'h{len(self.hnodes)}']))
        # backprop_1 = backprop_1.get()
        self.who += self.lr*backprop_1
        write(f"\n\nself.who: ", self.who)
        write(f"self.who.shape: ", self.who.shape)
        
        for i in range(len(self.hnodes), 1, -1):
            hidden_error = cp.dot(self.whh[f'wh{i-1}h{i}'].T, self.hidden_errors[f'h{i}'])
            self.hidden_errors[f'h{i-1}'] = hidden_error
            backprop_2 = cp.dot((self.hidden_errors[f'h{i}']*self.hidden_outputs[f'h{i}']*(1.0-self.hidden_outputs[f'h{i}'])), cp.transpose(self.hidden_outputs[f'h{i-1}']))
            # backprop_2 = backprop_2.get()
            self.whh[f'wh{i-1}h{i}'] += self.lr*backprop_2
        ...
        hidden_output = cp.dot((self.hidden_errors[f'h1']*self.hidden_outputs[f'h1']*(1.0-self.hidden_outputs[f'h1'])), cp.transpose(inputs))
        # hidden_output = hidden_output.get()
        self.wih += self.lr*hidden_output
    ...
    def query(self, input_list):
        ...
        inputs = cp.array(input_list, ndmin=2).T # u are using ndmin=2 because you want the inputs to be in row instead of col.
        ...
        hidden_output = cp.dot(self.wih, inputs)
        # hidden_output = hidden_output.get()
        self.hidden_outputs['h1'] = self.activation_function(hidden_output)
        ...
        for i in range(1, len(self.hnodes)):
            layer_number = i+1
            hidden_output = cp.dot(
                    self.whh[f'wh{i}h{i+1}'], # hidden to hidden layer mesh
                    self.hidden_outputs[f'h{i}'] # previous hidden layer outputs
                    )
            # hidden_output = hidden_output.get()
            self.hidden_outputs[f'h{layer_number}'] = self.activation_function(hidden_output)

        # calculate dot product from hidden layer to output layer with activation function applied
        final_output_dot = cp.dot(self.who, self.hidden_outputs[f'h{len(self.hnodes)}'])
        # final_output_dot = final_output_dot.get()
        final_output = self.activation_function(final_output_dot)

        formated_final_output = cp.array(final_output.T, ndmin=1)

        return final_output, cp.argmax(formated_final_output)
...
def extract_input_data(data_list = []):
    ...
    for data_str in data_list:
        ...
        pixel_list = (cp.asfarray(pixel_list)/255) * 0.99 + 0.01 # Here, to avoid saturation conditions we have used 0.99 and 0.01, 
        # e.g. if a 0 value came then 0/255 * 099 = 0 and this created a saturation of values in the model, so we added 0.01
        temp_target_list = cp.zeros(10)+0.01
...
if __name__ == "__main__":
    ...
    for input_, target in test_data:
        ...
        # break
        out_arr, label = n.query(input_)
        target_out = cp.argmax(target)
        # print(label, target_out)
        if int(label) == int(target_out):
            scorecard.append(1)
        else:
